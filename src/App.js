...
import EnglishSubtitles from './components/EnglishSubtitles';
...
export default function App({ defaultLang }) {
        ...
        if (localSubtitleString) {
            try {
                ...
                if (localSubtitle.length) {
                    ...
                    setEnglishSubtitles(localSubtitle.map((item) => new Sub(item)));
                } else {
                    fetchSubtitle();
                }
            } catch (error) {
    ...
    }, [setSubtitleOriginal,setEnglishSubtitles]);
